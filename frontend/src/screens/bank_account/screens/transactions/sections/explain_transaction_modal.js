...
class ExplainTransactionModal extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (this.props.selectedData !== prevProps.selectedData) {
      ...
      let transactionAmount = selectedData.withdrawalAmount ? +selectedData.withdrawalAmount : +selectedData.depositeAmount;
      this.setState({ transactionId: selectedData.id, currentBalance: transactionAmount, transactionAmount });
      ...
      for (let item in this.state.categoryList) {
        if (this.state.categoryList.hasOwnProperty(item)) {
          ...
          if (item === obj['categoryLabel']) {
            temp = this.state.categoryList[`${obj['categoryLabel']}`].filter((item) => item.id === obj.reconcileRrefId.value);
  ...
  checkChartOfAccount = () => {
    const { transactionAmount, currentBalance, submitBtnClick, transactionCategoryTypeList } = this.state
    if (transactionAmount > currentBalance && currentBalance !== 0 && submitBtnClick) {
      this.setState({
  ...
  render() {
    const { openExplainTransactionModal, closeExplainTransactionModal,selectedData } = this.props;
    const { chartOfAccountCategoryList, categoryList, showChartOfAccount, transactionAmount, currentBalance, explainList, transactionCategoryTypeList, submitBtnClick, showAlert, transactionCategoryList } = this.state
    return (
        ...
        >
          <ModalHeader toggle={this.toggleDanger}>
            <div className="header text">
              ...
              <i className="fa fa-close close-btn" onClick={() => closeExplainTransactionModal()}></i>
            </div>
          </ModalHeader>
          <ModalBody>
                      ...
                      <div class="sub-container">
                        <div className="mb-3 mr-2" style={{ width: '30%' }}>
                          ...
                          <Select
                            ...
                            onChange={(option) => {
                              ...
                              this.handleChange(option, 'chartOfAccountCategoryId', item)
                              this.handleChange('', 'transactionCategoryId', item)
                        ...
                        {explainList[`${index}`].chartOfAccountCategoryId && <div className="mb-3 mr-2" style={{ width: '30%' }}>
                          ...
                          <Select
                            ...
                            onChange={(option) => {
                              this.handleChange(option.label, 'categoryLabel', item)
                              this.handleChange(option, 'transactionCategoryId', item)
                              this.handleChange('', 'reconcileRrefId', item)
                        ...
                        {explainList[`${index}`].transactionCategoryId.value && <div className="mb-3" style={{ width: '40%' }}>
                          <Label className="label">{explainList[`${index}`].categoryLabel}</Label>
                          <Select
                            options={categoryList[`${explainList[`${index}`].categoryLabel}`] ? selectOptionsFactory.renderOptions('label', 'id', categoryList[`${explainList[`${index}`].categoryLabel}`], explainList[`${index}`].categoryLabel, ['isDisabled']) : []}
                            isOptionDisabled={(option) => option.isDisabled === true}
                            onChange={(val) => {
                              if (val && val.value) {
                                // this.getDetail(val.value)
                                if (this.checkCategory(val.value, 'reconcileRrefId')) {
                                  this.handleChange(val, 'reconcileRrefId', item, explainList[`${index}`].categoryLabel)
                                } else {
                                  this.handleChange('', 'reconcileRrefId', item, explainList[`${index}`].categoryLabel)
                                }
                              } else {
                                this.handleChange('', 'reconcileRrefId', item, explainList[`${index}`].categoryLabel)
                              }
                  ...
                  {transactionAmount > currentBalance && showChartOfAccount && (
                    <Row className="m-0">
                      ...
                      <Col lg={5} className="p-0">
                        <Label className="label">Transaction Category</Label>
                        <Select
                          options={transactionCategoryTypeList ? selectOptionsFactory.renderOptions('transactionCategoryName', 'transactionCategoryId', transactionCategoryTypeList, 'Type') : ''}
