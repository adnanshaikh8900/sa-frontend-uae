...
export const getCreditNoteList = (postObj) => {
	...
	return (dispatch) => {
		let param = `/rest/creditNote/getList?contact=${customerName}&type=7&referenceNumber=${referenceNumber}&amount=${amount}&status=${status}&pageNo=${pageNo}&pageSize=${pageSize}&order=${order}&sortingCol=${sortingCol}&paginationDisable=${paginationDisable}`;
		if (invoiceDate) {
