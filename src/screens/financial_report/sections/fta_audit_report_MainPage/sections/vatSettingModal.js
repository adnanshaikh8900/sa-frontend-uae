...
const mapDispatchToProps = (dispatch) => {
	return {
		...
		FTAAuditReportActions: bindActionCreators(FTAAuditReportActions, dispatch),
	};
};
const customStyles = {
	control: (base, state) => ({
...
class VatSettingModal extends React.Component {
	...
	handleSubmit = (data, resetForm, setSubmitting) => {
		...
		this.props.FTAAuditReportActions
			.VATSetting(formData)
			.then((res) => {
				if (res.status === 200) {
	...
	componentDidMount = () => {
		this.props.FTAAuditReportActions.getCompanyDetails().then((res)=>{			
			if(res.status==200){
