...
class ProductTable extends React.Component {
                    ...
                    <>
                        <Input
                            ...
                            onChange={(e) => {
                                ...
                                if (value === '' || (value !== '0' && this.regDecimal.test(value))) {
                                    this.selectItem(
                                        value,
                                        row,
                                        'unitPrice',
    ...
    renderVat = (cell, row, props) => {
        const vat_list = row.vat_list || [];
        const { strings, disableAll } = this.props;
        const idx = this.getIndex(row.id);
        console.log(row, 'skjhdk')
        return (
            <Field
                ...
                render={({ field, form }) => (
                    <>
                        <Select
                            ...
                            value={parseInt(row.vatCategoryId) === 10 ? { label: 'N/A', value: 10 } :
                                vat_list &&
                                selectOptionsFactory
                                    .renderOptions('name', 'id', vat_list, 'VAT')
                                    .find((option) => option.value === parseInt(row.vatCategoryId))
                            }
                            id="vatCategoryId"
        ...
        return (
            <Field
                ...
                render={({ field, form }) => (
                    <Select
                        styles={customStyles}
                        isDisabled={row.exciseTaxId === 0 || row.exciseTaxId === null || disableAll}
                        options={
                            excise_list
                                ? selectOptionsFactory.renderOptions(
                                    'name',
        ...
        data.map((obj, index) => {
            if (obj.id === row.id) {
                ...
                if (disableVat) {
                    ...
                    const vatCategory = vat_list && vat_list.find(obj => obj.id === result.vatCategoryId);
                    if (vatCategory)
                        obj['vatCategoryId'] = vatCategory.id;
                    else
                        obj['vatCategoryId'] = vat_list && vat_list.length > 0 ? vat_list[0]?.id : '';
                }
            }
            return obj;
        });
    ...
    renderAccount = (cell, row, props) => {
        const { strings, purchaseCategory, purchaseCategoryOptions, disableAll } = this.props;
        const idx = this.getIndex(row.id);
        return (
            <Field
                ...
                render={({ field, form }) => (
                    <Select
                        ...
                        value={
                            purchaseCategoryOptions ? purchaseCategoryOptions.find((obj) => obj.value === row.transactionCategoryId) : ''
                        }
    ...
    checkedRow = () => {
        ...
        if (data && data.length > 0) {
            ...
            let temp = data?.[length].productId !== "" ?
                data?.[length].productId : -2
            if (temp > -1) {
                return true;
            } else {
                return false;
